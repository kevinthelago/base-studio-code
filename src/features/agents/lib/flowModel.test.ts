...
import { flowSummary, depColor, stageColor } from "./flowModel";
...
describe("flowSummary (#1643)", () => {
  it("counts stalled + deadlocked and is idle when nothing is in flight", () => {
    expect(flowSummary([], [], {})).toEqual({ stalled: 0, deadlocked: 0, runEntries: [], idle: true });
  });

  it("tallies stalled/deadlocked views and reports runEntries, not idle", () => {
    const views = [view({ stalled: true }), view({ session: "t0p1", deadlocked: true })];
    const runs = { "#1": {} as WorkflowRun };
    const s = flowSummary(views, [waiter("t0p2")], runs);
    expect(s.stalled).toBe(1);
    expect(s.deadlocked).toBe(1);
    expect(s.runEntries).toHaveLength(1);
    expect(s.idle).toBe(false);
  });
});

describe("depColor / stageColor (#1643)", () => {
  ...
  it("maps run status to a color token", () => {
    expect(stageColor("done")).toBe("var(--success)");
    expect(stageColor("escalated")).toBe("var(--danger)");
    expect(stageColor("running")).toBe("var(--accent)");
  });
});
