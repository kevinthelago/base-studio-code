...
describe("mcpInstall helpers", () => {
  ...
  it("catalogLink returns the download link for downloadable first-party servers only", () => {
    expect(catalogLink("Plan Grader")).toMatch(/github\.com\/kevinthelago\/plan-grader-mcp-server$/);
    expect(catalogLink("Complexity Analyzer")).toMatch(/complexity-analyzer-mcp$/);
    expect(catalogLink("Compliance")).toBeUndefined(); // built-in native server (#1005), not downloadable
  ...
  it("mcpDirForServer builds the install path, '' for non-downloadable or empty base", () => {
    expect(mcpDirForServer("Plan Grader", "/b")).toBe("/b/mcp/plan-grader-mcp-server");
    expect(mcpDirForServer("Plan Grader", "")).toBe("");
    expect(mcpDirForServer("Compliance", "/b")).toBe(""); // built-in, not downloadable
    expect(mcpDirForServer("Postgres", "/b")).toBe("");
  });

  it("resolveMcpInstallDir substitutes {dir}; no-op without {dir}, link, or base", () => {
    expect(resolveMcpInstallDir({ args: "run --directory {dir} plan-grader-mcp" }, "Plan Grader", "/b").args)
      .toBe("run --directory /b/mcp/plan-grader-mcp-server plan-grader-mcp");
    // No baseDir → unchanged (resolved later when the base dir is known).
    expect(resolveMcpInstallDir({ args: "x {dir} y" }, "Plan Grader", "").args).toBe("x {dir} y");
