...
/**
 * Settings → Dependencies (#1916 follow-up). The one discoverable place that informs the user of every
 * host prerequisite the app + its agents need: the host tools (Git Bash · git · gh · gh-auth · the
 * agent CLI) via the preflight `DiagnosticsCard`, and — under the deny-list posture — the OS sandbox.
 *
 * Inform-first by design: each row shows its status and HOW to install it; nothing is installed
 * automatically. The host tools link to their official install pages; the sandbox's install is an
 * explicit button shown only after the gap is surfaced (with live progress), never a silent action.
 */
export function DependenciesPage() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18, maxWidth: 820 }}>
      <h2 className="mono" style={{ fontSize: 18, margin: "0 0 4px", fontWeight: 600 }}>Required dependencies</h2>
      <p style={{ color: "var(--fg-muted)", margin: "0 0 4px", fontSize: 12, lineHeight: 1.55 }}>
        ...
        row is for the default Claude Code runtime; the bundled <span className="mono">bsc-agent</span> shell
        needs no CLI.
      </p>
