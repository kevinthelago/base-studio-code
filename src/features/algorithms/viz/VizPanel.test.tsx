...
const viz = vizForImpl("sort.ts")!; // the seeded array-sort visualization
...
describe("VizStage — in-canvas take-over (#3205)", () => {
  ...
  it("Run re-runs the trace on the user's own input", () => {
    ...
    // The custom 3-element input renders a 3-cell trace.
    expect(stage.querySelector(".algo-viz-stage-body")!.querySelectorAll(".array-cell").length).toBe(3);
  });
  ...
  it("invalid input surfaces an error and keeps the last good run", () => {
    ...
    fireEvent.click(within(stage).getByRole("button", { name: "Run" }));
    expect(within(stage).getByText(/is not a number/i)).toBeTruthy();
