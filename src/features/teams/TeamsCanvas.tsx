...
function AgentFace({ d, isSel, dashed }: { d: PositionDisplay; isSel: boolean; dashed?: boolean }) {
  return (
    <Box style={{ width: "100%", height: "100%", boxSizing: "border-box", padding: "10px 12px",
      ...
      // Solid = shipped with the app; dashed = generated by the planner (#2750). Orthogonal to the
      // role color/width and the RoleDot — only the border STYLE changes.
      border: `1px solid ${isSel ? "var(--accent)" : "var(--border)"}`, borderStyle: dashed ? "dashed" : "solid",
      ...
      {org.positions.map((pos) => {
        ...
        return (
          ...
          <div key={pos.nodeId} data-node={pos.nodeId}
            ...
            {pos.kind === "agent" && <AgentFace d={d} isSel={isSel} dashed={pos.origin === "planner"} />}
            {pos.kind === "resource" && (
...
export function OrgLegend() {
  return (
    <Box style={{ position: "absolute", left: 16, bottom: 16, background: "color-mix(in oklch, var(--bg-elev) 92%, transparent)",
      ...
      {/* Node border style (#2750): solid = shipped with the app · dashed = generated by the planner. */}
      <Box style={{ height: 1, background: "var(--border-soft)", margin: "3px 0" }} />
      <Row gap={9} align="center">
        <svg width={30} height={14} style={{ flex: "none", overflow: "visible" }}>
          <rect x={1} y={2} width={12} height={10} rx={2} fill="none" stroke="var(--fg-dim)" strokeWidth={1.3} />
          <rect x={17} y={2} width={12} height={10} rx={2} fill="none" stroke="var(--fg-dim)" strokeWidth={1.3} strokeDasharray="3 2" />
        </svg>
        <Text as="span" mono size={9.5} tone="dim">solid = app · dashed = planner</Text>
      </Row>
    </Box>
  );
}
