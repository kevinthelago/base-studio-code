// The frontend ↔ org store bridge (#2193). The desktop Org designer and every live console session +
 ...
 */
export async function loadOrgs(): Promise<Org[] | null> {
  try {
    const out = await bsc(null, ["org", "list", "--full"]);
    const rows = JSON.parse(out.trim() || "[]") as Partial<Org>[];
    // Defensive: keep only well-formed rows (id present), defaulting the rest so an odd file never
    // crashes hydration.
    return (rows ?? [])
      .filter((o): o is Org => typeof o.id === "string" && !!o.id)
      .map((o) => ({
...
export async function pushOrg(org: Org): Promise<void> {
