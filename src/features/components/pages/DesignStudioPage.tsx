// Design Studio PAGE (#2668) — the component workbench, one of the three Design-Studio pages (Preview ·
// Design Studio · Theme Studio) the shell (`../DesignStudio.tsx`) switches between via the top-bar pills.
// This is the former DesignStudio body, unchanged except that the always-on designer terminal + the
// activity poll now live at the SHELL level (so the PTY survives page switches) and kit navigation is the
// left rail (the top bar navigates pages, not kits). Its own slim toolbar keeps search + share + add.
//
// It reuses the pure domain (`lib/model`), the shared specimen renderer (`renderSpecimen`), the shared
// graph stack (`GraphCanvas` + `ZoomControls` + `useGraphViewport` + `graphEdge`; the top-down hierarchy
// layout lives in `lib/compositionLayout`), and `useDragResize`. Data comes from the global store via the
// `bsc ui` bridge.
...
export function DesignStudioPage() {
  ...
  // The node the designer AI is currently working (#2525) — a `.working` pulse + auto-pan target. The
  // activity poll that sets it lives in the shell (always mounted with the terminal); this reads it.
  ...
  // The preview's THEME axis (#2488/#2545): the selected theme drives both the component retint (its
  // `vars`, via <ThemeScope>) AND the sandbox SURFACE (its `base`, below).
  ...
  return (
    <Box className="ds-page-design">
      {/* ── page toolbar — search + share + add (kit navigation is the rail; the top bar navigates pages) ── */}
      <Box className="ds-toolbar ds-subtoolbar">
      ...
      {shareOpen && (
        <KitShareModal
          ...
          onImported={(k) => { setKitId(k.id); setExpanded((e) => ({ ...e, [k.id]: true })); }}
        />
      )}
      ...
      {/* ── body — rail | composition graph | inspector ── */}
...
// ── Inspector ────────────────────────────────────────────────────────────────
// The full per-component detail surface (#2453): identity header · live preview (variant/theme/viewport +
// error card) · Overview/Source/Usage tabs.
interface InspProps {
