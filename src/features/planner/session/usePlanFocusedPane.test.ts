...
// reset-on-switch — plus that the pill derives from the SELECTED stage. The footer/prompt derivations
...
const stage = (key: string, status: Stage["status"] = "upcoming"): Stage => ({
  key, name: key, glyph: "x", blurb: "", gate: "", index: 0, total: 3, status, fraction: 0,
});

const base: PlanFocusedPaneOpts = {
  stages: [stage("goal", "complete"), stage("scope", "active"), stage("stack")],
  focusActiveIdx: 1,
  planComplete: false,
...
describe("usePlanFocusedPane", () => {
  it("auto-follows the active stage when nothing is pinned", () => {
    const { result } = render();
    expect(result.current.focusSelectedIdx).toBe(1); // = focusActiveIdx
    expect(result.current.focusPill).toBe("wait");   // the active stage isn't complete/ahead
  });

  it("pins to a user selection and derives that stage's pill", () => {
    const { result } = render();
    act(() => result.current.setFocusSel(0));
    expect(result.current.focusSelectedIdx).toBe(0);
    expect(result.current.focusPill).toBe("pass");   // stage 0 is complete
  });

  it("clamps an out-of-range selection to the stage list", () => {
    const { result } = render();
    act(() => result.current.setFocusSel(99));
    expect(result.current.focusSelectedIdx).toBe(2); // clamped to the last index
  });

  it("resets the selection on a project switch (re-follows the active stage)", () => {
