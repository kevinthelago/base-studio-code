...
const SKILLS = [
  skill("s1", "API design", "Prefer REST; version every endpoint."),
  skill("k1", "House style", "2-space indent; no default exports."),
];

describe("resolveSkillContent (#636 slice b)", () => {
  it("pulls content from skills (prompt), skipping unknown ids", () => {
    const out = resolveSkillContent(["s1", "ghost", "k1"], SKILLS);
    expect(out).toEqual([
      { name: "API design", kind: "skill", content: "Prefer REST; version every endpoint." },
      { name: "House style", kind: "skill", content: "2-space indent; no default exports." },
    ]);
  });
});
...
describe("buildSkillContext (#636 slice b)", () => {
  ...
  it("renders project-wide + per-stage attached skills with their content", () => {
    ...
    const doc = buildSkillContext(bp({ skills: ["k1"], sections: [api] }), SKILLS);
  ...
  it("returns empty string when nothing is attached", () => {
    expect(buildSkillContext(bp({ sections: [mkStageSection("api")] }), SKILLS)).toBe("");
  });
});
