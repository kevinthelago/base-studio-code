...
describe("previewRender (#2623 — the verify-build → preview-node seam)", () => {
  ...
  it("renders a native binary as a frame stream", () => {
    expect(previewRender({ kind: "native", streamId: "s1" })).toEqual({ mode: "stream", streamId: "s1" });
  });

  it("splits iframe-renderable sources from the native frame-stream", () => {
    const web: PreviewSource = { kind: "web", url: "u" };
    const bundle: PreviewSource = { kind: "bundle", srcDoc: "d" };
    const native: PreviewSource = { kind: "native", streamId: "s" };
