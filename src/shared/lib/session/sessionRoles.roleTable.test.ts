...
import { ROLE_DEFAULTS, PLANNER_WRITE_GLOBS, DB_OWNED_PLAN_FILES, DEP_MANIFEST_FILES } from "./sessionRoles";
...
describe("role capability table (loaded from @data/permissions/role-capabilities.json)", () => {
  ...
  it("only the planner ships default write globs; every other role starts empty (no code writes)", () => {
    ...
    for (const c of Object.values(ROLE_DEFAULTS)) {
      if (c.role !== "planner") expect(c.writeGlobs).toEqual([]);
    }
  });
