...
const RESET_STATE = {
  ...
  configProfiles: [] as import("./").ConfigProfile[],
...
describe("agent fleet store", () => {
  ...
  it("gives the director AND every worker the project's MCP servers (#876)", () => {
    useAppStore.setState({
      ...
      mcpServers: [{
        id: "mcp_comp", name: "Compliance", enabled: true, projects: ["mcp-proj"],
...
describe("mcp servers store", () => {
  it("add (assigns id) / toggle / update / setProjects / remove", () => {
    ...
    const s = useAppStore.getState();
    s.addMcpServer({ name: "fs", enabled: false, projects: [], transport: "stdio", command: "npx", args: "" });
    const id = useAppStore.getState().mcpServers[0].id;
    expect(useAppStore.getState().mcpServers).toHaveLength(1);
    expect(id).toMatch(/^mcp_/);
    s.toggleMcpServer(id);
    expect(useAppStore.getState().mcpServers[0].enabled).toBe(true);
    s.updateMcpServer(id, { command: "node" });
    expect(useAppStore.getState().mcpServers[0].command).toBe("node");
    s.setMcpServerProjects(id, ["P1"]);
    expect(useAppStore.getState().mcpServers[0].projects).toEqual(["P1"]);
    s.removeMcpServer(id);
    expect(useAppStore.getState().mcpServers).toHaveLength(0);
  });

  it("fleetStartProject resolves only enabled global + this-project servers onto panes", () => {
    const servers: McpServer[] = [
      { id: "g",     name: "g", enabled: true,  projects: [], transport: "stdio" },
      { id: "p",     name: "p", enabled: true,  projects: ["proj-key"], transport: "stdio" },
      { id: "off",   name: "x", enabled: false, projects: [], transport: "stdio" },
      { id: "other", name: "o", enabled: true,  projects: ["zzz"], transport: "stdio" },
    ];
    useAppStore.setState({ bscBaseDir: "/base", mcpServers: servers });
    const fleet: FleetPlan = {
      recommended: 1, reasoning: "", director: { enabled: false },
      streams: [{ id: "s0", name: "S0", repo: "own/web", owns: [], issues: [], dependsOn: [] }],
    };
    useAppStore.getState().fleetStartProject("ExtP", fleet, "proj-key");
    // director disabled ⇒ pane 0 is the single worker "s0".
    const ids = (useAppStore.getState().paneMcpServers[fleetPaneId("proj-key", "s0")] ?? []).map(e => e.id);
...
describe("hooks store", () => {
  it("add (assigns id) / toggle / update / setProjects / remove", () => {
    useAppStore.setState({ hooks: [] });
    const s = useAppStore.getState();
    s.addHook({ name: "fmt", enabled: false, projects: [], event: "PostToolUse", matcher: "Write", command: "fmt" });
    const id = useAppStore.getState().hooks[0].id;
    expect(useAppStore.getState().hooks).toHaveLength(1);
    expect(id).toMatch(/^hook_/);
    s.toggleHook(id);
    expect(useAppStore.getState().hooks[0].enabled).toBe(true);
    s.updateHook(id, { command: "prettier" });
    expect(useAppStore.getState().hooks[0].command).toBe("prettier");
    s.setHookProjects(id, ["P1"]);
    expect(useAppStore.getState().hooks[0].projects).toEqual(["P1"]);
    s.removeHook(id);
    expect(useAppStore.getState().hooks).toHaveLength(0);
  });
});
...
describe("deleteLocalProject — resilient to a missing projects scope (#791)", () => {
  ...
  it("does not throw when a server/hook/skill has no `projects` field, and normalizes it to []", () => {
    // Mimic a def added without `projects`, or persisted data predating the field.
    useAppStore.setState({
      mcpServers: [{ id: "mcp_x", name: "X", enabled: true } as unknown as McpServer],
      hooks: [{ id: "hook_x", name: "H", enabled: true } as unknown as Hook],
      skills: [{ id: "sk_x", name: "Y" } as unknown as Skill],
    });

    expect(() => useAppStore.getState().deleteLocalProject(["some-project"])).not.toThrow();
    expect(useAppStore.getState().mcpServers[0].projects).toEqual([]);
    expect(useAppStore.getState().hooks[0].projects).toEqual([]);
