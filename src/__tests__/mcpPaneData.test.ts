...
import { buildMcpServers } from "../screens/projects/mcpPaneData";
...
describe("buildMcpServers", () => {
  it("includes globals + project-scoped MCP servers, excludes other projects and non-mcp", () => {
    const exts: ExtensionDef[] = [
      ...
      { id: "h", kind: "hook", name: "Guard", enabled: true, projects: [], event: "PreToolUse", hookCommand: "x" },
    ];
    expect(buildMcpServers(exts, "proj", fleet).map((s) => s.id)).toEqual(["g", "p"]);
  });
