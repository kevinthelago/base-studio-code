...
import { runCleanupScan } from "./cleanupScan";
...
describe("runCleanupScan (#626 slice c)", () => {
  ...
  it("with an API key, verifies + confirms candidates (lowers the score)", async () => {
    routeInvoke();
    const out = await runCleanupScan({ projectKey: "p", sectionKey: "cleanup", repoPath: "/r", stack: "js", llm: { provider: "anthropic", model: "claude-sonnet-4-6", apiKey: "sk-x", baseUrl: "" } });
