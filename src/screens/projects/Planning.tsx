...
import { deriveLifecycleState, LIFECYCLE_LABEL, buildRefactorFleet, buildRefactorIssues } from "../../lib/planLifecycle";
...
export function Planning({ visible }: { visible: boolean }) {
  ...
  // Lifecycle state (#458): new → active → near-complete, derived from the live
  // issue closure ratio (paneData.phaseStructure) and the plan grade score.
  const lifecycleState = useMemo(() => {
    const totalIssues  = paneData.phaseStructure.reduce((sum, p) => sum + p.total, 0);
    const closedIssues = paneData.phaseStructure.reduce((sum, p) => sum + p.closed, 0);
    return deriveLifecycleState({
      isExisting:     isExisting,
      totalIssues,
      closedIssues,
      planGradeScore: paneData.grade?.score,
    });
  }, [isExisting, paneData.phaseStructure, paneData.grade?.score]);
  ...
  // Refactor pass mode (#458): activated when the project is near-complete.
  const [refactorPassActive, setRefactorPassActive] = useState(false);
  ...
  // Refactor pass (#458): configure the fleet + inject pre-built refactor issues
  // into the plan so the Triage / Fleet launch can immediately execute them.
  function activateRefactorPass() {
    if (publishRepos.length === 0) return;
    const fleet  = buildRefactorFleet(publishRepos);
    const issues = buildRefactorIssues(publishRepos);
    const store  = useAppStore.getState();
    store.setPlanFleet(effectiveProjectId, fleet);
    // Append refactor issues to issues.json.
    const existing = sections.find(s => s.k === "issues")?.content ?? "[]";
    let parsedArr: object[] = [];
    try { parsedArr = JSON.parse(existing.trim() || "[]"); } catch { parsedArr = []; }
    const merged = [...parsedArr, ...issues];
    store.setPlanSection(effectiveProjectId, "issues", JSON.stringify(merged, null, 2));
    store.confirmPlanSection(effectiveProjectId, "issues");
    setRefactorPassActive(true);
  }
      ...
      <div style={{ padding: "12px 24px 12px", display: "flex", alignItems: "flex-start", gap: 14 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            ...
            <span className={"tag " + (lifecycleState === "near-complete" ? "green" : "amber")}>
        ...
        {lifecycleState === "near-complete" && !refactorPassActive && (
          <button
            className="btn ghost"
            onClick={activateRefactorPass}
            disabled={publishRepos.length === 0}
            title="Configure a refactor/optimization pass for this near-complete project"
          >
            Refactor pass →
          </button>
        )}
