...
import { autopilotProgress, type AutopilotStrategy } from "./planAutopilot";
import { usePlanAutopilot, type AutopilotDeps, type AutopilotLogEntry } from "./planAutopilotRunner";
...
export function Planning({ visible }: { visible: boolean }) {
  ...
  const [autopilotStrategy, setAutopilotStrategy] = useState<AutopilotStrategy>("llm");
  // The FEATURE stops at a publishable plan (false); the dev harness mock-publishes (true).
  ...
  const autopilotDeps: AutopilotDeps = {
    ...
    snapshot: () => {
      ...
      return {
        ...
        plannerAwaiting,
        working: grew,
        autoPublish: autopilotAutoPublish,
    ...
    mockPublish: () => { /* harness: never touch real GitHub — the run logs "publish (mocked)" */ },
    log: (e) => setAutopilotLog(prev => [...prev, e]),
  };
  const autopilot = usePlanAutopilot(autopilotDeps);
...
_Auto-generated by base-studio-code planner._`,
  ...
  return (
    <>
      {/* Planning autopilot (#682) — DEV ONLY. Drives the session with a simulated user;
          publish is mocked. A test/automation harness, not a shipped feature. */}
      {import.meta.env.DEV && (
        <div style={{
          position: "fixed", bottom: 12, right: 12, zIndex: 90, width: 280, maxHeight: 240,
          display: "flex", flexDirection: "column", gap: 6, padding: 10, borderRadius: 10,
          background: "var(--bg-elev)", border: "1px solid var(--border-soft)", boxShadow: "0 8px 24px rgba(0,0,0,.35)",
          fontFamily: "var(--mono)", fontSize: 10.5,
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ color: "var(--accent)", fontWeight: 700 }}>autopilot</span>
            <span style={{ color: "var(--fg-dim)" }}>dev · mock publish</span>
            <span style={{ flex: 1 }} />
            <select value={autopilotStrategy} disabled={autopilot.running}
              onChange={(e) => setAutopilotStrategy(e.target.value as AutopilotStrategy)}
              style={{ fontFamily: "var(--mono)", fontSize: 9.5, background: "var(--bg-canvas)", color: "var(--fg)", border: "1px solid var(--border-soft)", borderRadius: 5, padding: "2px 4px" }}>
            ...
            <button className="btn sm"
              onClick={autopilot.running ? autopilot.stop : () => { setAutopilotAutoPublish(true); autopilot.start(); }}
              disabled={!autopilot.running && autopilotStrategy === "llm" && !claudeApiKey}
              title={autopilotStrategy === "llm" && !claudeApiKey ? "Add a Claude API key in Settings → Integrations" : "harness: mock publish"}>
              {autopilot.running ? "■ stop" : "▶ run"}
            </button>
          </div>
          <div style={{ color: "var(--fg-muted)" }}>
            tick {autopilot.state.iteration} · {Math.round(autopilotProgress(phases).fraction * 100)}% complete
            {autopilot.state.result && (autopilot.state.result.completed
              ? <span style={{ color: "var(--success)" }}> · ✓ done</span>
              : <span style={{ color: "var(--danger)" }}> · stalled: {autopilot.state.result.stalledReason}</span>)}
          </div>
          {autopilotLog.length > 0 && (
            <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column", gap: 2, color: "var(--fg-dim)" }}>
              {autopilotLog.slice(-8).map((e, i) => (
                <div key={i} style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  <span style={{ color: "var(--fg-muted)" }}>{e.action}</span>{e.detail ? ` · ${e.detail}` : ""}
                </div>
              ))}
            </div>
          )}
      ...
      <div style={{ padding: "12px 24px 12px", display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        {/* Auto-plan (#682) — the Settings feature: Claude drives planning to a publishable
            plan for your review (never auto-publishes). Clean header control, no panel. */}
        {autoPlanWithClaude && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, alignSelf: "center" }}>
            ...
            <button
              className="btn"
              onClick={autopilot.running ? autopilot.stop : () => { setAutopilotStrategy("llm"); setAutopilotAutoPublish(false); autopilot.start(); }}
              disabled={!autopilot.running && !claudeApiKey}
              title={claudeApiKey ? "Let Claude drive planning to a publishable plan for your review" : "Add a Claude API key in Settings → Integrations"}
            >{autopilot.running ? "■ stop auto-plan" : "✦ Auto-plan"}</button>
          </div>
        )}
      </div>
      {triageBlock && (
