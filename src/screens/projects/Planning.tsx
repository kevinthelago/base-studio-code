...
export function Planning({ visible }: { visible: boolean }) {
  ...
  const planReady = useMemo(() => {
    const fleet = planFleet[effectiveProjectId];
    return PLAN_STAGES.filter(st => !st.optional).every(st => isStageGateMet(st, sections, linkedRepos, fleet));
  }, [sections, linkedRepos, planFleet, effectiveProjectId]);
  ...
  async function handleRestart() {
    ...
    refreshSetupSig(); // baseline updated after a regenerating restart (#756)
      ...
      <div style={{ padding: "14px 24px", display: "flex", alignItems: "flex-start", gap: 14 }}>
        ...
        {contextStale && (
          <button
            ...
            disabled={restarting}
            onClick={async () => { await handleRestart(); }}
            title="The project's inputs (repos / KB / blueprint stages) or the planner template changed since this session started — regenerate its context (#175)"
