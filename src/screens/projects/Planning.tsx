...
import { BlueprintLibrary } from "./blueprints/BlueprintLibrary";
...
export function Planning({ visible }: { visible: boolean }) {
  ...
  // Blueprint library (#662 #670 #664).
  const [showBlueprints, setShowBlueprints] = useState(false);
  const [blueprintId, setBlueprintId] = useState<string | null>(null);
  // #664: backfill — if this project already has content, treat it as "pre-tracking"
  // (a "default" sentinel) so the reset modal fires when any blueprint is applied.
  const blueprintBackfilledRef = useRef(false);
  useEffect(() => {
    if (!blueprintBackfilledRef.current && sections.some(s => s.state !== "pending")) {
      blueprintBackfilledRef.current = true;
      setBlueprintId(prev => prev === null ? "default" : prev);
    }
  }, [sections]);
...
_Auto-generated by base-studio-code planner._`,
  ...
  return (
    <>
      ...
      {/* Blueprint library panel (#662 #670 #664) — shown between rail and split panel */}
      {showBlueprints && (
        <BlueprintLibrary
          activeBlueprintId={blueprintId}
          onUse={id => { setBlueprintId(id); setShowBlueprints(false); }}
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0, overflow: "hidden", borderTop: "1px solid var(--border-soft)" }}>
        ...
        <section style={{ flex: "1 1 0", display: "flex", flexDirection: "column", minHeight: 0, overflow: "hidden", borderRight: "1px solid var(--border-soft)" }}>
          <div style={{
            ...
            <button
              data-testid="blueprint-toggle"
              onClick={() => setShowBlueprints(v => !v)}
              style={{
                padding: "2px 8px", borderRadius: 3, cursor: "pointer",
                background: showBlueprints ? "var(--accent-dim)" : "transparent",
                border: "1px solid " + (showBlueprints ? "var(--accent)" : "var(--border-soft)"),
                color: showBlueprints ? "var(--accent)" : "var(--fg-dim)",
                fontFamily: "var(--mono)", fontSize: 10,
              }}
            >
              {blueprintId && blueprintId !== "default" ? `✓ ${blueprintId}` : "blueprint ▾"}
            </button>
            <button
