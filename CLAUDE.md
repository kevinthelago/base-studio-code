...
(`tester`/`reviewer`/`conductor` are the pipeline-stage roles, #220; `issuer` is intake-only — shapes a request into a GitHub issue and hands off, #376; `juror` independently judges a landing against its acceptance criteria, #394.) `roleDeniedCommands` denies the mutating git/gh commands a role cannot run; `roleWriteRules` denies/scopes the file-write tools. The session allows Bash broadly and guarantees `gh`/`git` on PATH; Claude Code precedence is **deny > ask > allow**. (Moving to a **deny-list model** under #1916: sessions run `bypassPermissions` with a deny-list + per-session worktree filesystem isolation (`bsc-scope`) as the primary control, and the enumerated allow-list becomes an opt-in posture toggle.)
