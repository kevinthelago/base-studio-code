...
// The PTY-byte / native↔bash path-conversion / ANSI-C-quoting half (#1300) lives in a sibling
// module. Re-exported here so every existing `crate::platform::shell::<name>` path (and the
// prelude re-exports) still resolves unchanged after the split (#2110).
mod ptybridge;
pub(crate) use ptybridge::*;
