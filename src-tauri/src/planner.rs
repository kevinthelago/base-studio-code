...
fn stage_directive(id: &str) -> String {
    let line = match id {
        ...
        "structure"   => "**Structure** — the features are a dependency DAG (`bsc-plan feature list` shows each feature + its `dependsOn`). Two passes: (1) sequence the DAG into ordered phases (`phases.json`, foundations first — a feature's deps land in the same or an earlier phase) and get the user's approval; (2) decompose each feature IN DEPENDENCY ORDER — `bsc-plan feature get <slug>` to pull its spec, then emit its granular agent-ready issues `echo '{…,\"stream\":\"<slug>\",\"dependsOn\":[…]}' | bsc-plan add` (one per issue; the issue `dependsOn` realizes the feature edges — point at issues in the features it builds on). Go ONE feature at a time; never move on until the current feature is fully decomposed and stored. (Existing repos: inventory every screen/module first so none is missed.)",
...
mod tests {
    ...
    fn stage_directive_structure_mentions_workshop_modes() {
        ...
        assert!(d.to_lowercase().contains("dependency order"), "missing the DAG-ordered decomposition");
        assert!(d.contains("bsc-plan feature get"), "missing the per-feature pull");
        assert!(d.contains("inventory"), "missing existing-project handling");
        assert!(d.contains("ONE feature"), "missing pace mandate");
    }
