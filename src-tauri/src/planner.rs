...
fn stage_directive(id: &str) -> String {
    let line = match id {
        ...
        "deploy"      => "**Deploy** (right after Repos) — define how each service SHIPS: the deploy TARGET (hosting platform + workload — static/serverless/container/long-running) per service, the ENVIRONMENT ladder (dev → staging → prod, each mapped to a branch), the CI/CD PIPELINE (build → test → deploy, with the gating stage), CONFIG + SECRETS per env (names only — values live in the host vault), the RELEASE strategy (recreate/rolling/blue-green/canary) + rollback, and HEALTH checks. Propose sensible defaults from the stack, interrogate the user, and let them set targets in the Deploy pane. It lands as deployment issues owned by a `deploy` stream at publish.",
