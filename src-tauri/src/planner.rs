...
fn stage_directive(id: &str) -> String {
    let line = match id {
        ...
        "structure"   => "**Structure** — the features are a dependency DAG (`bsc-plan feature list` shows each feature + its `dependsOn`). SEQUENCE them into a roadmap; do NOT write issues — issues are generated from the features at GitHub-publish time, not during planning (no `bsc-plan add`, no issue files). Group the features into ordered phases (foundations first — a feature's deps land in the same or an earlier phase): (1) write `phases.json`; (2) assign EVERY feature its phase via `echo '{\"slug\":\"…\",\"phase\":<n or name>}' | bsc-plan feature add`. Then get the user's approval. (Existing repos: inventory every screen/module first so none is missed.)",
...
mod tests {
    ...
    fn stage_directive_structure_sequences_features_without_authoring_issues() {
        ...
        assert!(d.contains("phases.json"), "missing the phase sequencing");
        assert!(d.to_lowercase().contains("do not write issues"), "must NOT author issues (publish-time, #plan-db)");
