...
fn stage_directive(id: &str) -> String {
    let line = match id {
        ...
        "features"    => "**Features** — work titles-first via the `bsc-plan feature` store (NOT a features.json file). STEP 1: register the COMPLETE title roster in one pass — `bsc-plan feature add \"Invite teammates\" \"Export to CSV\" ...` (names only; the board shows each as an undefined title). Agree the roster with the user before detailing. STEP 2: drive ONE feature at a time, filling its detail by slug — `echo '{\"slug\":\"invite-teammates\",\"behavior\":\"…\",\"acceptance\":[\"…\"],\"approach\":\"…\",\"tools\":[\"…\"],\"data\":\"…\",\"stream\":\"…\"}' | bsc-plan feature add` (merges in place — do NOT resend the name). `behavior` = what it does + when, in the user's terms; `acceptance` = a done-when checklist; `data` = what it stores/reads + which features it depends on; `stream` defaults to the slug. A feature is defined once it has name + behavior + ≥1 acceptance (`bsc-plan feature list` shows ✓/·). Do NOT design the integration contracts here — that's the Plan/Structure stage. When EVERY feature is populated, present the set and let the USER confirm to complete the stage — never advance the stage yourself.",
        "structure"   => "**Structure** — with the features defined, run the workshop to synthesize the plan (new project → feature-by-feature; existing → section-by-section migration — inventory every screen/module first), then write `phases.json` + add agent-ready issues to the plan store (`echo '{…}' | bsc-plan add`, one per issue — NOT an issues.json file). Go ONE unit at a time; never move on until the current unit is fully decomposed and stored.",
