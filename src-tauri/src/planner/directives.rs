...
pub(crate) fn stage_directive(id: &str) -> String {
    let line = match id {
        ...
        "ui"          => "**UI** — runs AFTER Features: capture the screens that deliver the defined capabilities as a PLAN, not as code. **Do NOT design or generate the screens yourself** — no skeleton code, no live preview; the visual design comes from the user via Claude Design. Author a **Claude Design kickoff** at `prompts/ui-kickoff.md` — a self-contained brief (product goal, feature→screen map, each screen's purpose/states/flows, design-system constraints) concrete enough that Claude Design can produce the screens without re-asking — and write `ui.md` with the screen inventory. **If the project has a Data Model** (a `datamodel.json` in the hub, from a migration/data project), AUTHOR THE KICKOFF FROM IT: map each entity → its screens (list / detail / form), the relationships → navigation between them, and fold the **Platform Behavior Summary** captured in the Source scan (validation rules, Flows/Process Builder, approval processes) into the screens' states/flows + validation — so Claude Design produces data-driven screens over the real schema, not generic ones. If the user has DROPPED design files (check `design/intake.json` — the file-intake manifest), they're real Claude Design exports: route each into the UI repo (e.g. `<repo>/src/components/`) and reference them in `prompts/ui-kickoff.md` AND in the UI stream's worker kickoff — so the fleet builds the actual approved design. Then tell the user to take the kickoff to Claude Design and drop the exported `.dc.html` files back into the UI panel, which routes the design and clears the stage.",
...
mod tests {
    ...
    fn ui_directive_routes_design_without_generating_it() {
        ...
        assert!(ui.contains("datamodel.json"), "ui directive must drive the kickoff from the Data Model when present: {ui}");
