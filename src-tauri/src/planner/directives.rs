...
pub(crate) fn stage_directive(id: &str) -> String {
    let line = match id {
        ...
        "deploy"      => "**Deploy** (right after Repos) — define how each service SHIPS, then RECORD it in the plan DB by piping the config JSON to `bsc-plan deploy set`. **`bsc-plan deploy set` is what clears the gate and fills the Deploy pane — a prose `deploy.md` does NOT** (`bsc-plan deploy get` shows the stored config). JSON: `services` — an array with ONE per repo, EACH A SELF-CONTAINED DEPLOYABLE UNIT (#1421). A service REQUIRES `platform` + `workload` = static|serverless|container|service (plus `id`, `repo`, `region`, `build`, `output`/`runtime`) AND carries its OWN: `environments` (≥2; each `name`, `branch`, `url`, `auto`), `pipeline` (`provider` + `stages`: array of `{name, trigger: push|tag|on-green|manual, gate: bool, cmd}`, ≥2 stages), `secrets` (array of `{key, envs:[…]}` — list `prod` in `envs` for every prod-needed secret), `release` (`strategy` = recreate|rolling|blue-green|canary, `autoRollback`, `keep`, `migrateWithDeploy`), and `health` (`probe`, `slo`, `alerts`). Then LOCK the dependency manifest with `bsc-plan deps set` (manifest JSON on stdin, replaces the whole manifest): a `dependencies` array — each `{repo, ecosystem: npm|cargo, name, version, dev?, why, source?}` — plus a `registries` map defining any non-public `source` (`url`, `scope`, `auth` = the token's SECRET NAME, also recorded in the deploy `secrets` above). Lock them ONCE here so the parallel fleet never each redefines libraries (`bsc-plan deps get` shows the stored manifest). The gate needs BOTH halves: `deploymentDefined` — EVERY repo's service is deploy-ready (a target = `platform` or its local fields, ≥2 of its own environments, ≥2 pipeline stages, its prod secrets wired, a non-empty `release.strategy`) — AND `dependenciesDefined`: ≥1 dependency locked via `bsc-plan deps set`. Propose defaults from the stack, confirm with the user, then pipe the whole config: `echo '{…}' | bsc-plan deploy set` (re-run with the full config as it firms up). A human-readable `deploy.md` is optional reference. Publishes as deployment issues owned by a `deploy` stream.",
        "ui"          => "**UI** — runs AFTER Features: capture the screens that deliver the defined capabilities as a PLAN, not as code. **Do NOT design or generate the screens yourself** — no skeleton code, no live preview; the visual design comes from the user via Claude Design. Author a **Claude Design kickoff** at `prompts/ui-kickoff.md` — a self-contained brief (product goal, feature→screen map, each screen's purpose/states/flows, design-system constraints) concrete enough that Claude Design can produce the screens without re-asking — and write `ui.md` with the screen inventory. If the user has DROPPED design files (check `design/intake.json` — the file-intake manifest), they're real Claude Design exports: route each into the UI repo (e.g. `<repo>/src/components/`) and reference them in `prompts/ui-kickoff.md` AND in the UI stream's worker kickoff — so the fleet builds the actual approved design. Then tell the user to take the kickoff to Claude Design and drop the exported `.dc.html` files back into the UI panel, which routes the design and clears the stage.",
        ...
        "repos_deploy" => "**Deployment** — ONE stage, two parts. (1) **Link** the repositories this project spans: emit `<repo_link owner/repo>` for each (clones into the hub + records the link in plan.db; `bsc-plan repo add owner/repo`, `bsc-plan repo list`). (2) **Ship**: define how each service DEPLOYS and the libraries it depends on, recorded in plan.db — `bsc-plan deploy set` (config JSON: `services`, ONE per repo — each a self-contained deployable unit carrying its OWN `platform`+`workload`, ≥2 `environments`, a staged `pipeline`, `secrets`, and a `release.strategy`) and `bsc-plan deps set` (the locked dependency manifest). The gate needs BOTH: ≥1 repo linked AND every repo's service deploy-ready + ≥1 locked dependency. `bsc-plan deploy get` / `bsc-plan deps get` show the stored config.",
...
mod tests {
    ...
    #[test]
    fn deploy_directive_covers_shipping_and_dependencies() {
        // #1412: the standalone Deploy stage's gate needs BOTH deploymentDefined AND ≥1 locked
        // dependency (sections/deploy.json) — so the overview directive must instruct both halves,
        // not just `bsc-plan deploy set`. Keeps it in sync with the merged `repos_deploy` directive.
        let dep = stage_directive("deploy");
        assert!(dep.contains("bsc-plan deploy set"), "deploy directive records the deploy config: {dep}");
        assert!(dep.contains("bsc-plan deps set"), "deploy directive must also lock the dependency manifest: {dep}");
        assert!(dep.contains("dependenciesDefined"), "deploy directive's gate must name the dependency half: {dep}");
    }
