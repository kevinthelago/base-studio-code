...
describe("vitest discovery excludes: content", () => {
  ...
  it("excludes the e2e/ Playwright harness (#3264/#3411)", () => {
    // e2e/ is deliberately outside the default gate, but its specs match vitest's default
    // `*.spec.ts` include glob. The exclusion was documented in CLAUDE.md yet never listed here, so a
    // root run collected `e2e/previewInteraction.spec.ts` and the suite died on `@playwright/test` —
    // a browser dep the zero-install worktree workflow is specifically not meant to require.
