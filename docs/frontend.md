...
- **`terminal/`** — the **single-terminal host** (#2378, epic #2372). `TerminalHost` (mounted once per window in `App.tsx`/`DetachedWindow.tsx`) owns exactly ONE `TerminalView` per paneId, portaled into a stable per-pane container. A pane cell and the Glance dock each drop a `TerminalSlot paneId` placeholder; the host **re-parents** that one container into whichever slot currently owns it (console grid cell OR Glance dock), parking it in a hidden holding node otherwise — so the xterm/PTY move between surfaces intact, never a second mount. This replaced the pre-#2378 double-mount (two `TerminalView`s coordinating by hand so they wouldn't fight over PTY sizing).
