...
| [`console/`](../src-tauri/src/console/) | The interactive PTY execution surface | `pty/` (dir-module, #1660 — `pty/mod.rs`: `pty_create` + the `pty_*` commands, the session env; `pty/job.rs`: the Windows Job Object / Unix process-group tree kill), `ledger.rs` (boot-time orphan reaping), `discovery.rs` (durable session discovery), `shell_rc.rs` (the `bsc-*` shell-helper rc constants) |
| [`session/`](../src-tauri/src/session/) | How an agent session is launched, configured & permissioned (one directory, #1917) | `harness.rs` (`HarnessAdapter` seam — `ClaudeCodeAdapter` vs `BscAgentAdapter`), `launch.rs` (`claude_launch`, model flags, history probes), `claude_config.rs` (read/write + self-heal `~/.claude.json` trust state), `settings.rs` (`ensure_session_settings` → `.claude/settings.json`, the role gate, the `DEFAULT_DENY`/mandatory baseline), `llm.rs` (the `llm_complete` one-shot completion → `crates/llm`) |
...
| [`data`](../crates/data/) (`bsc-data`) | `bsc_data` | **`bsc-data`** (needs `duckdb-store`) | Canonical Data Model store (DuckDB) + the connector framework — a single `RestConnector` over an **agent-authored manifest** (the native per-vendor connectors + `presets.rs` were removed, #1976; the manifest is the sole connector path). Schema/DDL/connector/coercion logic compiles under `--no-default-features` (no DuckDB); the planner reads the per-project model/scan via `bsc-data model get` / `bsc-data scan get`, and authors connectors via the `bsc-data connector` dev-loop (`probe`/`validate`/`try`/`map`, #1963 — which **replaced** the deprecated `bsc-plan integration`, #1721). |
...
### Session settings & the role gate — `ensure_session_settings` ([`session/settings.rs`](../src-tauri/src/session/settings.rs))
...
| `bsc-data` | `data` | `$BSC_DATA_BIN` | Read the per-project Data Model + PlatformScan (`bsc-data model get` / `bsc-data scan get`) and author connector manifests via the dev-loop (`bsc-data connector probe`/`validate`/`try`/`map`, #1963 — which replaced the deprecated `bsc-plan integration`, #1721). |
