...
impl Store {
    ...
    //    per (repo, mode)); durable in plan.db instead of a <startup_script> stream tag, so the
    //    section poll reflects them into the store and the planner drives them via `bsc plan startup
    //    add/list/remove`. ──
