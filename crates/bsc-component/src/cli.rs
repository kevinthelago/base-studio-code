...
Prints the stored component JSON for <id> verbatim, or `null` if absent. With --field, prints just the
value at the RFC-6901 JSON pointer <json-pointer> (e.g. `/name`, `/props/0/req`, `/srcText`; a leading
`/` is optional) — errors when the component OR the field is absent. --raw unwraps a string value (no
JSON quotes/escaping) so a shell `$(...)` capture is clean — a non-string value still prints as compact
JSON; without --raw the value prints as JSON (--pretty indents).",
    },
    CmdDoc {
        name: "set",
...
pub fn run(args: Vec<String>, prog: &str) -> Result<(), String> {
    ...
    match args.first().map(String::as_str) {
        ...
        // BEFORE touching stdin/the store (in its own fn, like `doctor --fix`).
        Some("set-src") => {
            if args.get(1).map(String::as_str) == Some("help") {
        ...
        // store CLI rejects unknown flags); a plain `get <id>` still delegates unchanged. A read verb.
...
/// SAME write-time JSX syntax gate as `set` (#2928) before writing. Errors when the component is absent.
/// Pure of stdin/print so tests drive it directly.
...
/// `patch /srcText …` can't smuggle in a module that won't build. Errors when the component (or the
/// pointer's parent) is absent. Pure of stdin/print so tests drive it directly.
fn apply_patch(
    store: &bsc_json_store::Store,
    id: &str,
    pointer: &str,
...
/// Render ONE resolved field value for `get --field`: `raw` unwraps a string (no quotes/escaping) so a
/// shell capture is clean — a non-string value still prints as compact JSON; otherwise JSON (pretty
/// indents). Pure → unit-tested.
...
fn cmd_get_field(args: &[String]) -> Result<(), String> {
    ...
    println!("{}", field_output(value, raw, pretty)?);
    Ok(())
}
    ...
    fn command_docs_expose_the_component_surface_for_the_ui_mount() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "define-animation", "list-animations", "remove-animation", "set-src", "patch"
            ]
        );
