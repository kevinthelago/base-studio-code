...
- `bsc log` — toggle CONSOLE log scopes at runtime while debugging: `bsc log set fleet off` quiets a
  subsystem's console output, `bsc log set console::pty debug` lights one up, `bsc log list` shows the
  graph. The file/audit log always keeps everything — this only changes what the console VIEW shows.
