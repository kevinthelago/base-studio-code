/* global React, ReactDOM, FLEET, Titlebar, Rail, ModeStrip, StatusBar, Avatar, CardHead, StatCard, RangeToggle, useTip, LineArea, Bars, Donut, HBars, Swimlane, Legend, fmt */
    ...
    <div className="card" style={{
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <div style={{ flex: 1, fontSize: 12, lineHeight: 1.6, color: "var(--fg-muted)" }}>
          ...
          <p style={{ margin: 0 }}>
            6 workers on <b style={{ color: "var(--fg)" }}>Settlement webhooks v2</b> — 3 running, on pace for the iteration.
            Landed <b style={{ color: "var(--success)" }}>7 issues</b> today; director merged 9 PRs.
...
// ── worker status board ──────────────────────────────────────────────────────
function WorkerBoard({ selected, onSelect }) {
  return (
    <div className="card">
      <CardHead title="Worker board" hint="one agent per stream · own worktree + branch"
        right={<span style={{ fontFamily: "var(--mono)", fontSize: 10.5, color: "var(--accent)" }}>{F.WORKERS.length} workers live</span>} />
      <div style={{ borderRadius: 6, border: "1px solid var(--border-soft)", overflow: "hidden" }}>
        <div style={{
          display: "grid", gridTemplateColumns: "120px 92px 1fr 120px 64px 56px",
          gap: 10, padding: "7px 12px", background: "var(--bg-elev2)",
          borderBottom: "1px solid var(--border-soft)",
          fontFamily: "var(--mono)", fontSize: 9.5, color: "var(--fg-dim)",
          textTransform: "uppercase", letterSpacing: ".05em",
        }}>
          <span>worker</span><span>status</span><span>current</span><span>stream progress</span><span style={{ textAlign: "right" }}>tokens</span><span style={{ textAlign: "right" }}>last</span>
        </div>
        {F.WORKERS.map((w, i) => {
          const st = F.STATUS[w.status];
          const pr = F.PROFILE[w.profile];
          const on = selected === w.id;
          return (
            <div key={w.id} className="hrow" onMouseEnter={() => onSelect(w.id)} style={{
              display: "grid", gridTemplateColumns: "120px 92px 1fr 120px 64px 56px",
              gap: 10, padding: "9px 12px", alignItems: "center", fontSize: 11,
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "var(--mono)", fontSize: 9, color: "var(--fg-dim)", marginBottom: 3 }}>
                  <span>{w.ownedDone}/{w.ownedTotal}</span><span>{Math.round(w.ownedDone / w.ownedTotal * 100)}%</span>
                </div>
                <div className="meter"><i style={{ width: `${w.ownedDone / w.ownedTotal * 100}%`, background: pr.color }} /></div>
              </div>
      ...
      <div style={{ marginTop: 10 }}>
        <Legend items={Object.entries(F.STATUS).map(([k, s]) => ({ color: s.color, label: s.label }))} />
      </div>
    </div>
  );
}
...
function CoordTimeline() {
  ...
  return (
    <div className="card">
      ...
      <Swimlane lanes={F.COORD_LANES} events={F.COORD_EVENTS} marks={F.COORD_MARKS} tip={tip} height={26} />
...
function FleetStatus() {
  ...
  const slices = Object.entries(counts).map(([k, v]) => ({ name: F.STATUS[k].label, value: v, color: F.STATUS[k].color }));
  return (
    <div className="card">
      <CardHead title="Fleet status" hint="right now" />
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <Donut slices={slices} center={{ value: F.WORKERS.length, label: "workers" }} />
...
// ── cost & token burn ────────────────────────────────────────────────────────
function CostBurn() {
  const rows = [...F.WORKERS].sort((a, b) => b.tokensK - a.tokensK).map(w => ({
    label: w.id, value: w.tokensK, color: F.PROFILE[w.profile].color,
  ...
  const totalK = F.WORKERS.reduce((s, w) => s + w.tokensK, 0);
  const totalUsd = F.WORKERS.reduce((s, w) => s + w.costUsd, 0);
  return (
    <div className="card">
...
function DirectorCard() {
  ...
  const tone = { green: "var(--success)", running: "var(--accent)", blocked: "var(--fg-dim)" };
  return (
    <div className="card">
      <CardHead title="Director · merge queue" hint={`drive: ${d.drive} · idle ${d.idleFor}m`}
        right={<span className="tag green" style={{ fontSize: 9 }}>{d.mergedToday} merged today</span>} />
      <div style={{ display: "flex", flexDirection: "column", gap: 1, borderRadius: 6, border: "1px solid var(--border-soft)", overflow: "hidden" }}>
        {queue.map((q, i) => (
          <div key={q.pr} className="hrow" style={{
            display: "grid", gridTemplateColumns: "42px 1fr 58px", gap: 8, alignItems: "center",
            padding: "9px 11px", fontSize: 11, background: i % 2 ? "var(--bg-panel)" : "var(--bg-elev)",
          }}>
...
function CycleTime() {
  ...
  return (
    <div className="card">
      ...
      <Bars labels={F.CYCLE_BUCKETS.map(b => b.label)}
        groups={[{ name: "issues", color: "var(--accent)", data: F.CYCLE_BUCKETS.map(b => b.v) }]}
        height={120} tip={tip} />
...
function FleetView() {
  ...
  return (
    <div className="app">
      <Titlebar workspace="Projects — Settlement webhooks v2 — fleet"
        meta={[{ label: "fleet", value: "6 live" }, { label: "branch", value: "develop" }]} />
      <div className="shell">
        <Rail active="projects" />
        <div className="main">
          <ModeStrip active="fleet" sync="github sync · fleet live"
            modes={[
              { k: "summary", label: "Summary", href: "#" },
              { k: "projects", label: "Projects", href: "#" },
              { k: "fleet", label: "Fleet", hint: "live orchestration", href: "Fleet Analytics.html" },
            ]} />
          <div className="page">
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 14 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: "var(--fg-muted)", fontSize: 12, marginTop: 4 }}>
                      Settlement webhooks v2 · 6 workers across 6 streams · director coordinating
                    </div>
                  </div>
          ...
          <StatusBar items={[
            { tone: "", text: "fleet running · 3 active" },
            { tone: "warn", text: "2 awaiting you" },
            { tone: "off", text: "director idle 1m" },
          ]} />
        </div>
      </div>
    </div>
...
ReactDOM.createRoot(document.getElementById("root")).render(<FleetView />);
