...
### Fixed
- **Tab/section tear-off broke the new window on Windows** — detached windows were created from the JS `new WebviewWindow()` API, which (a) can't set `additionalBrowserArgs`, so the torn-off window's WebView2 launched with different args than the main window and failed the shared-user-data-dir constraint (the window crashed), and (b) encoded the detach target in a URL query, which the webview layer rewrites/strips for app pages so the bundle never loaded and the window came up blank white. They're now built from a Rust `open_detached_window` command that applies the same browser args (drift-guarded against `tauri.conf.json`) and loads the **same plain `index.html` the main window loads** (so the bundle loads and React mounts identically); the detach target is delivered as a `window.__BSC_DETACH__` global injected by an init script before page scripts run, never via the URL (#1870)
